...
// ═══ 전역 상태
let allData          = [];
let kbMap            = {};     // key: "sido||sgg||apt_norm||area반올림"  or  "_fb||apt_norm||area반올림"
...
// ═══ 아파트명 정규화 (KB↔실거래 매칭용)
// 미성(1차)→미성1차, 한양(2단지)→한양2단지, 공백제거
function normalizeApt(name) {
    let n = (name || '').trim();
    n = n.replace(/\((\d+차)\)/g,   '$1');
    n = n.replace(/\((\d+단지)\)/g, '$1');
    n = n.replace(/\s+/g, '');
    return n;
}
...
function loadData() {
    const guard = setTimeout(() => {
        hideLoader();
        showEmpty('⏱ 로딩 시간 초과\napt_trade_data.csv 파일이 index.html과 같은 폴더에 있는지 확인하세요.');
    }, 60000);
    ...
    fetchText('map.csv').then(text => {
        ...
        rows.forEach(r => {
            ...
            const aptOrig = (r['apt']      || '').trim();
            const aptNorm = (r['apt_norm'] || normalizeApt(aptOrig)).trim();
            const area    = parseFloat(r['area'] || 0);
            if (!aptNorm || !area) return;
            const n = s => parseInt(String(s || '0').replace(/,/g,'')) || 0;
            const entry = { 하한가:n(r['하한가']), 일반거래가:n(r['일반거래가']), 상한가:n(r['상한가']), area, sido, sigungu:sgg, apt:aptOrig };
            const ar = Math.round(area);
            // 정밀키: sido+sgg+apt_norm+area
            const k1 = `${sido}||${sgg}||${aptNorm}||${ar}`;
            if (!kbMap[k1]) kbMap[k1] = [];
            kbMap[k1].push(entry);
            // 폴백키: apt_norm+area
            const k2 = `_fb||${aptNorm}||${ar}`;
...
function dateKey(ym, day) {
    // YYYYMMDD 숫자 반환
    return parseInt(String(ym) + String(day).padStart(2, '0'));
}
function getFilteredData() {
    let data = allData;
    if (filterFrom > 0 || filterTo > 0) {
        data = data.filter(d => {
            const dk = dateKey(d.계약년월, d.계약일);
            if (filterFrom > 0 && dk < filterFrom) return false;
            if (filterTo   > 0 && dk > filterTo)   return false;
            return true;
        });
    }
    return data;
...
function updateStats(data) {
    ...
    const p = src.map(d => d.price).filter(Boolean);
    if (!p.length) {
        ['stat-avg','stat-med','stat-max'].forEach(id => { const e=g(id); if(e) e.textContent='—'; });
        return;
    }
...
function setupEvents() {
    ...
    document.querySelectorAll('.modal-tab').forEach(t => t.addEventListener('click', function() {
        ...
        g('panel-' + this.dataset.tab).classList.remove('hidden');
        recalc();
    }));
...
function applyDateFilter() {
    const from = g('date-from').value; // "YYYY-MM"
    const to   = g('date-to').value;
    filterFrom = from ? parseInt(from.replace('-','') + '01') : 0;
    filterTo   = to   ? parseInt(to.replace('-','')   + '31') : 0;

    // 유효성: 최대 1년 이내
    if (from && to) {
        const fDate = new Date(from + '-01');
        const tDate = new Date(to   + '-01');
        const diffMs = tDate - fDate;
        const diffDays = diffMs / (1000*60*60*24);
        const errEl = g('date-error');
        if (diffDays < 0) {
            if (errEl) errEl.textContent = '종료일이 시작일보다 빠릅니다.';
            return;
        }
        if (diffDays > 366) {
            if (errEl) errEl.textContent = '최대 1년 이내로 설정하세요.';
            return;
        }
...
function renderList() {
    ...
    const noData = (filterFrom > 0 || filterTo > 0) && !filtered.length;
    if (noData) {
        grid.innerHTML = `
<div class="empty-state">
  <div class="empty-icon">📅</div>
  <p class="empty-msg" style="color:var(--text2)">선택한 기간에 해당하는 거래가 없습니다</p>
  <p class="empty-msg" style="font-size:.75rem;color:var(--text3);margin-top:.5rem">기간을 조정하거나 필터를 변경해 보세요</p>
</div>`;
        return;
    }
    if (!filtered.length) {
...
function kbRefLabel(floor) { return parseInt(floor)===1 ? 'KB하한' : 'KB일반'; }
...
function cardDefault(d) {
    ...
    const kbLine = kb
        ? `<div class="kb-compare-row">${kbChipHtml(d.price,kb,d.층)}<span class="kb-chip neutral">${kbRefLabel(d.층)} ${f억(kr)}</span></div>`
        : `<div class="kb-compare-row"><span class="kb-chip neutral" style="font-size:.58rem;color:var(--text3)">시세없음</span></div>`;
...
function openHistory(name, sido, gudong) {
    ...
    let kbLine = '';
    if (kb) {
        kbLine = `<div style="background:var(--surface3);border:1px solid var(--border);border-radius:.5rem;padding:.5rem .875rem;margin-bottom:.75rem;font-size:.72rem;font-weight:700;color:var(--text2)">KB 하한 <b style="color:var(--text1)">${f억(kb.하한가)}</b> · 일반 <b style="color:var(--blue)">${f억(kb.일반거래가)}</b> · 상한 <b style="color:var(--text1)">${f억(kb.상한가)}</b></div>`;
    } else {
        kbLine = `<div style="background:var(--surface3);border:1px solid var(--border);border-radius:.5rem;padding:.5rem .875rem;margin-bottom:.75rem;font-size:.72rem;font-weight:600;color:var(--text3)">KB 시세 없음</div>`;
    }
    if (!list.length) {
...
function openM(name, tradePrice, kbPrice, regStatus, floor) {
    ...
    if (currentKbPrice > 0) {
        kbValEl.textContent = f억(currentKbPrice);
        kbValEl.style.color = '';
    } else {
        kbValEl.textContent = '시세없음';
        kbValEl.style.color = 'var(--text3)';
    }
...
function calcLoan() {
    ...
    g('calc-cash').textContent   = '필요 자본금: ' + fW(cash);
...
// index.html의 onclick="closeModal(...)" 호환
