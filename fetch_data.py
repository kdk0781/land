...
수정사항:
  - 전국 17개 시도, ~264개 지역코드로 확장
  - sigungu 포맷: '수원시' 통일 (구 단위가 있으면 시만, 없으면 그대로)
    → update_all.py, kb.json 매칭 기준과 일치
"""
...
# ─── 지역코드 맵 ──────────────────────────────────────────────
# 형식: '코드': ('시도', '시군구')
# 시군구는 CSV sigungu 컬럼 기준 → "시" 단위로 통일
# (예: 수원장안구/권선구/팔달구/영통구 → 모두 '수원시')
DISTRICT_MAP = {
    # ── 서울특별시 ──────────────────────────────────────────
    ...
    try:
        ...
        for item in root.findall('.//item'):
            ...
            if url_type == 'rent' and d.get('전월세구분') != '전세':
                continue
            d['sido']    = sido
            d['sigungu'] = sigungu   # ← 통일된 시 단위 sigungu
...
all_t, all_r = [], []
total = len(DISTRICT_MAP)
print(f"전국 {total}개 지역코드 데이터 수집 시작...")

for i, (code, (sido, sigungu)) in enumerate(DISTRICT_MAP.items(), 1):
    for m in months:
...
print(f"매매 {len(all_t)}건, 전세 {len(all_r)}건 수집 완료")
...
if all_t:
    ...
    # 컬럼 정리
    df = df.rename(columns={'면적_r': '전용면적_r'})
    ...
    print(f"완료! 총 {len(out):,}건 저장 → apt_trade_data.csv")
