...
let filterTo         = 0;
let loadedFiles      = 0;
const TOTAL_FILES    = 4; // seoul, gyeonggi_a, gyeonggi_b, incheon
...
function parseRows(json) {
    ...
    return json.data.map(row => {
        ...
        const price=row[8], jPrice=row[9], gap=row[10], jRatio=row[11];
        ...
        return {
            ...
            price:price||0, gap:gap||0, jeonsePrice:jPrice||0, jeonseRatio:jRatio||0,
            pyung:pyung.toFixed(1),
            pyungPrice:pyung>0?Math.round((price||0)/pyung):0,
            아파트:apt, aptNorm:aptN||normApt(apt),
            전용면적:String(area), 층:String(floor),
            계약년월:String(ym), 계약일:String(day),
        };
    });
}

// ═══ 데이터 로드 (4개 파일 순차, 첫 파일 즉시 렌더)
function loadData() {
    const FILES = [
        'trade_seoul.json',
        'trade_incheon.json',
        'trade_gyeonggi_a.json',
        'trade_gyeonggi_b.json',
    ];
    ...
    const guard = setTimeout(() => {
        hideLoader();
        if (!allData.length) showEmpty('⏱ 로딩 시간 초과\ntrade_*.json 파일들이 index.html과 같은 폴더에 있는지 확인하세요.');
    }, 30000);

    setLoader('서울 데이터 로드 중...');

    // 파일별 순차 fetch (첫 파일 완료 즉시 화면 표시)
    const loadFile = (idx) => {
        if (idx >= FILES.length) {
            clearTimeout(guard);
            return;
        }
        const labels = ['서울','인천','경기(1/2)','경기(2/2)'];
        setLoader(`${labels[idx]} 데이터 로드 중... (${idx+1}/${FILES.length})`);

        fetch(FILES[idx])
            .then(r => {
                if (!r.ok) throw new Error(`${FILES[idx]} 로드 실패 (${r.status})`);
                return r.json();
            })
            .then(json => {
                ...
                if (!firstLoaded) {
                    ...
                    // 이후 파일은 카운트만 업데이트
                    g('total-count').textContent = loadedFiles < TOTAL_FILES
            ...
            .catch(err => {
                clearTimeout(guard);
                hideLoader();
                if (!allData.length) showEmpty('❌ ' + err.message);
            });
    };
...
function updateSidoSelect(){ initSidoSelect(); }
...
function cardDefault(d){
    ...
    const kbl=parseInt(d.층)===1?'KB하한':'KB일반';
  ...
  <div class="kb-compare-row">${kbChipHtml(d.price,kb,d.층)}${kb&&kr?`<span class="kb-chip neutral">${kbl} ${f억(kr)}</span>`:''}</div>
...
function cardVolume(d,idx){
  ...
  <div class="card-name">${d.아파트}</div><div class="card-meta">${d.sido} ${d.gudong}</div>
  <div class="card-bottom">
...
function cardGap(d,idx){
  ...
  <div class="card-name">${idx+1}. ${d.아파트}</div><div class="card-meta">${d.전용면적}㎡ · ${d.pyung}평</div>
...
function cardCompare(d,idx){
  ...
  <div class="card-name">${d.아파트}</div><div class="card-meta">${d.전용면적}㎡ · ${d.pyung}평 · ${d.층}층</div>
...
function openHistory(name,sido,gudong){
    ...
    g('history-list').innerHTML=kbLine+(list.length?list.map(d=>{
        const ym=String(d.계약년월),dt=`${ym.slice(0,4)}.${ym.slice(4)}.${String(d.계약일).padStart(2,'0')}`;
        const maxP=Math.max(...list.map(x=>x.price));
...
function calcLoan(){
    ...
    const loan=Math.min(ltvLim,dsrLim/(factor*12),tP>250000?20000:tP>150000?40000:60000);
